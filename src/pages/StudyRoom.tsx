...
import { Check, X, RefreshCcw, ArrowLeft, BrainCircuit, Edit3, Sparkles, Volume2, VolumeX, Type, Pin, PinOff, Minimize2, Maximize2, Play, Pause, Clock, BellPlus, Trash2, Plus, AlertCircle, BarChart3, Activity as ActivityIcon } from "lucide-react";
...
export default function StudyRoom() {
  ...
  useEffect(() => {
    ...
    let unsubscribe = () => {};
    try {
      unsubscribe = onSnapshot(doc(db, "sets", deckId), (docSnap) => {
        if (docSnap.exists()) {
        ...
        <div className={cn("w-full h-full transition-all duration-500 transform-style-3d rounded-3xl", isFlipped ? "[transform:rotateY(180deg)]" : "")}>
          ...
          <div className="absolute inset-0 backface-hidden glass rounded-3xl text-center h-full">
            <div className="w-full h-full flex flex-col items-center justify-center p-8">
              ...
              {!isEditing ? (
                <h2 className="text-4xl font-display font-semibold text-center">{currentCard.front}</h2>
              ) : (
